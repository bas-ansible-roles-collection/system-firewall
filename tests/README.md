...
* Enables rules associated with the firewall application (service) for SSH

Multiple scenarios are tested *manually*:

1. `test-bare` - Enables rules associated with the firewall application (service) for SSH
2. `test-port-test` - Used for functional tests to verify the theoretical state of the system firewall is true 
practically
...
```shell
mv ~/ansible-system-firewall ~/system-firewall
cd tests
pip install ansible
```
...
available [here](https://semaphoreci.com/antarctica/ansible-system-firewall).
...
| ------------------------------------- | ----------------- | ------------------------------------------- | --------------------------- |
| barc-system-firewall-test-ubuntu-bare | *computed value*  | `barc-system-firewall-test-ubuntu-bare.v.m` | The VM's private IP address |
| barc-system-firewall-test-centos-bare | *computed value*  | `barc-system-firewall-test-centos-bare.v.m` | The VM's private IP address |
| barc-system-firewall-test-port-test   | *computed value*  | `barc-system-firewall-test-port-test.v.m`   | The VM's private IP address |
